...
bool AudioSystem::Initialize()
{
	...
	LoadBank("Assets/Bank/Master Bank.strings.bank");
	LoadBank("Assets/Bank/Master Bank.bank");
	return true;
}
...
void AudioSystem::SetListener(const Matrix4& viewMatrix, const Vector3& currentPos, float deltaTime, const Vector3& listenerPosition)
{
	...
	listener.position = VecToFMOD(listenerPosition);
	listener.forward = VecToFMOD(invView.GetZAxis());
	listener.up = VecToFMOD(invView.GetYAxis());

	// Calculate velocity (v = (currentPos - prevPos) / deltaTime)
	Vector3 velocity(((currentPos.x - mPreviousPos.x) / deltaTime), ((currentPos.y - mPreviousPos.y) / deltaTime), ((currentPos.z - mPreviousPos.z) / deltaTime));

	// Set velocity (convert to FMOD vector if necessary)
